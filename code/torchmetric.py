...
class SequentialPerturbation:
    def __init__(self, model, masker, sort_order, perturbation, linearize_link=False):
        # self.f = lambda masked, x, index: model(masked)
        # self.f = lambda masked, x, index: model(masked[0])
        self.f = lambda masked, x, index: model(masked.to(device))
        # self.f = lambda masked, x, index: model(torch.from_numpy(masked[0]).float())
        # self.f = lambda masked, x, index: model(torch.from_numpy(np.transpose(masked[0], (2, 0, 1))).float())
    ...
    def score(self, explanation, X, percent=0.01, y=None, label=None, silent=False, debug_mode=False):
        ...
        for i in range(len(X)):
            if self.data_type == "image":
                # print("X:",X[i].shape)
                # print("attributions:",attributions[i].shape)
                x_shape, y_shape = attributions[i].shape[0], attributions[i].shape[1]
                feature_size = np.prod([x_shape, y_shape])
                sample_attributions = attributions[i].mean(2).reshape(feature_size, -1)
                #找到模型预测最大的那个值所在的索引
                # index = np.argmax(self.model(X[i][np.newaxis, ...]))
                x = normalize(X)
                # print("model:",self.model(x).shape)
                # index = np.argmax(self.model(x).detach().numpy()[i])
                # print("index:",index)
                # print("predict:",self.model(x).detach().numpy()[i][0,index])
            else:
            ...
            for k in range(output_size):
                ...
                # masked = np.transpose(masked[0], (0, 3, 1, 2))
                # masked = torch.from_numpy(masked).float()
                # masked = self.masker(mask, normalize(X[i][np.newaxis, ...]))
                data = None
                masked = normalize_mask(masked[0])
                # curr_val = [self.f(masked, data, k)[0,index].item()]
                if self.perturbation == "remove":
                    curr_val = [-torch.max(self.f(masked, data, k)).item()]
                else:
                    curr_val = [torch.max(self.f(masked, data, k)).item()]
                # curr_val = [torch.min(self.f(masked, data, k)).item()]
                max_index = torch.argmax(self.f(masked, data, k))
        ...
        for j in range(len(self.score_values[-1])):
            xp = np.linspace(0, 1, len(self.score_values[-1][j]))
            yp = self.score_values[-1][j]
            # curves[j, :] = np.interp(xs, xp[:len(yp)], np.ravel(yp))
            curves[j,:] = np.interp(xs, xp, yp)
        ys = curves.mean(0)
